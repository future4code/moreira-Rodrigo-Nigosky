console.log ("hello world")

//*****EXERCICIO INTERPRETAÇÃO

//*****EXERCICIO 1

//a. undefined 
//b. null
//c. 11
//d. 3
//e.( 11) [3, 19, 5, 6, 7, 8, 9, 10, 11, 12, 13]
//f. 9


//******EXERCICIO 2

//R: "SUBI NUM ONIBUS EM MIRROCOS" 27




//****EXERCICIO ESCRITA CODIGO

//*********EXERCICIO  1.******
/* const nomeDoUsuario = prompt ("Qual seu nome?")
const emailDoUsuario = prompt ("Qual seu email?")
const fraseTemplate = `O email ${emailDoUsuario} foi cadastrado com sucesso. Seja bem-vinda(o), ${nomeDoUsuario}`

console.log (fraseTemplate)
 */

//**********EXERCICIO 2.******

//a.
/*  const comida = ["frango", "pizza", "bolo", "carne", "peixe"]
console.log (comida) */


//b.
/* let comida = ["frango", "pizza", "bolo", "carne", "peixe"]
console.log (comida)

const comidaUsuario = prompt ("Qual sua comida favorita?")

console.log (`Essas são suas comidas preferidas: 
${comida[0]}, 
${comida[1]}, 
${comida[2]}, 
${comida[3]}, 
${comida[4]} ` 
)
*/


//c. 
/* 
let comida = ["frango", "pizza", "bolo", "carne", "peixe"]
console.log (comida)

const comidaUsuario = prompt ("Qual sua comida favorita?")

console.log (`Essas são suas comidas preferidas: 
${comida[0]}, 
${comida[1]}, 
${comida[2]}, 
${comida[3]}, 
${comida[4]} `
)

comida[1] = comidaUsuario
console.log(comida)

 */


//**********EXERCICIO 3******
 
//a.
/* let listaDeTarefas = []

//b.
const tarefasUsuario1 = prompt("cite uma tarefa que voce precise realizar no dia a dia") 
const tarefasUsuario2 = prompt("cite outra tarefa que voce precise realizar no dia a dia") 
const tarefasUsuario3 = prompt("cite uma ultima tarefa que voce precise realizar no dia a dia") 

listaDeTarefas.push(tarefasUsuario1,tarefasUsuario2, tarefasUsuario3)

//c.
console.log (listaDeTarefas)

//d.
const tarefaRealizada = Number (prompt("digite o índice de uma tarefa que ele já realizou: 0, 1 ou 2"))

//e.
listaDeTarefasNova = listaDeTarefas.splice(tarefaRealizada, 1)

//f.
console.log (listaDeTarefas) 
console.log (listaDeTarefasNova)     */
     


//****EXERCICIO DESAFIO

//**********EXERCICIO 1******

/* const frase = ["vamos"+"dormir"+"agora"]
console.log(frase) */


/* ***rascunho****
const frase = prompt("Escreva uma frase")
console.log(frase)
console.log (frase.length) 
console.log(frase[0])
 */

//**********EXERCICIO 2******

/* 
const frutas = ["Banana", "Morango", "Abacaxi", "Laranja", "Ameixa"]
const Abacaxi = [3 -1]

//indice da palavra abacaxi
console.log(Abacaxi)

//imprimir o indice

//tamanho da array
console.log(frutas.length)

 */
